...
const TIERS = [
  { key: "good", label: "Good", highlight: false, border: "rgba(0,0,0,0.09)", bg: "#fff" },
  { key: "better", label: "Better", highlight: true, border: "#d4521a", bg: "#fffaf8" },
  { key: "best", label: "Best", highlight: false, border: "rgba(0,0,0,0.09)", bg: "#fff" },
];
...
export default function ProposalViewPage() {
  ...
  useEffect(() => {
    fetch(`/api/proposals/${id}`)
      ...
      }).catch(() => { toast.error("Failed to load"); router.push("/dashboard/workspace"); })
    ...
    <div style={{ padding: "40px 40px 60px", maxWidth: 900, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#999" }}>
          ...
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", background: ss.bg, color: ss.color, padding: "3px 10px", borderRadius: 100 }}>{proposal.status}</div>
        </div>
      ...
      <div style={{ background: "#eaf6f0", border: "0.5px solid #a3d9bc", borderRadius: 14, padding: "16px 24px", marginBottom: 28, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontSize: 14, color: "#1a5c35" }}>Better tier = <strong>${o.tiers.better.price - proposal.input.currentRate} more</strong> than your old flat rate</div>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 24 }}>
          <div style={{ fontSize: 28, fontWeight: 800, letterSpacing: -1, color: "#0f5c35", lineHeight: 1 }}>+${o.revenueLiftEstimate.conservative}–${o.revenueLiftEstimate.optimistic}</div>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          {TIERS.map(({ key, label, highlight, border, bg }) => {
            const tier = o.tiers[key as keyof typeof o.tiers];
            return (
              <div key={key} style={{ background: bg, border: `0.5px solid ${border}`, borderRadius: 16, padding: 22, position: "relative", boxShadow: highlight ? "0 4px 20px rgba(212,82,26,0.12)" : "none" }}>
                ...
                <div style={{ fontSize: 32, fontWeight: 800, letterSpacing: -1, color: highlight ? "#d4521a" : "#0a0a0a", lineHeight: 1, marginBottom: 16 }}>${tier.price}</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {tier.deliverables.map((d: string, i: number) => (
                    <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: 13, color: "#3a3a3a" }}>
                      <span style={{ color: "#0f5c35", flexShrink: 0 }}>✓</span>{d}
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 28 }}>
        ...
        <div style={{ background: "#f5f4f0", borderRadius: 14, padding: 20 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", color: "#666", marginBottom: 10 }}>Urgency + Closing</div>
          <p style={{ fontSize: 14, color: "#0a0a0a", lineHeight: 1.7, margin: "0 0 12px" }}>&quot;{o.urgencyLine}&quot;</p>
      ...
      <div style={{ background: "#0a0a0a", borderRadius: 14, padding: 20, marginBottom: 28 }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", color: "rgba(240,237,232,0.40)", marginBottom: 8 }}>Suggested Email Subject</div>
      ...
      <div style={{ background: "#fff", border: "0.5px solid rgba(0,0,0,0.09)", borderRadius: 14, padding: 24, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "#666" }}>Copy email template → paste into Gmail → send.</div>
        </div>
        <button onClick={copyEmailTemplate} style={{ background: copiedEmail ? "#0f5c35" : "#d4521a", color: "#fff", border: "none", padding: "12px 24px", borderRadius: 100, fontSize: 14, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0, marginLeft: 20 }}>
...
function ActionBtn({ onClick, label, color, ghost }: { onClick: () => void; label: string; color: string; ghost?: boolean }) {
  return (
    <button onClick={onClick} style={{ padding: "9px 18px", borderRadius: 100, border: ghost ? `0.5px solid ${color}` : "none", background: ghost ? "transparent" : color, color: ghost ? color : "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      {label}
    </button>
  );
}
  ...
  return (
    <div style={{ padding: "40px 40px", maxWidth: 900, margin: "0 auto" }}>
      {[200, 120, 300, 200].map((h, i) => (
        <div key={i} style={{ height: h, borderRadius: 16, background: "#ede9e4", marginBottom: 16 }} />
      ))}
    </div>
  );
}
